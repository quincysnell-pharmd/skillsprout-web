...
function MatchingStep({ step, onComplete }: { step: Step; onComplete: () => void; }) {
  ...
  function select(side: "left"|"right", id: string, value: string) {
    ...
    // Find the correct right-side value for the selected left item
    ...
    // Match by value — so identical right-side words all count as correct
...
function JournalStep({ step, childId, lessonId, courseId, onComplete, isCompleted }: {
  ...
  return (
    <div className="space-y-4">
      ...
      />
      {saved ? (
        <div className="rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-sm font-bold text-yellow-800">
...
// ── Reflection Step ───────────────────────────────────────────
function ReflectionStep({ step, childId, courseId, lessonId, onComplete }: { step: Step; childId: string; courseId: string; lessonId: string; onComplete: () => void; }) {
  const supabase = supabaseBrowser();
  const [text, setText]         = useState("");
  const [submitted, setSubmitted] = useState(false);
...
function StepContent({ step, childId, courseId, lessonId, onComplete, isCompleted }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* INTERACTIVE — now handled via Reflection's share button, keeping for backward compat */}
